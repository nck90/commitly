...
export default function SignupPage() {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (signRes?.error) {
                toast.error("자동 로그인에 실패했습니다. 직접 로그인 해주세요.");
                router.push("/login");
            } else {
                router.refresh(); // AuthContext route handler will direct to onboarding
            }
        } finally {
            setIsLoading(false);
        }
        ...
        <div className="min-h-screen bg-background flex flex-col items-center justify-center relative overflow-hidden">
            ...
            >
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-4xl font-extrabold tracking-tight mb-3">Join <span className="text-primary">Commitly</span></h1>
                    <p className="text-muted-foreground text-lg">새로운 차원의 외주 관리를 시작하세요</p>
                </div>
                ...
                <div className="glass-card p-8 sm:p-10 !rounded-[2rem] border-white/20 dark:border-white/10 shadow-2xl backdrop-blur-2xl bg-white/40 dark:bg-black/40">
                    <form onSubmit={handleSignup} className="space-y-5">
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-foreground">이름 또는 회사명</label>
                    ...
                    <div className="mt-8 text-center text-sm font-medium text-muted-foreground">
                        이미 계정이 있으신가요? <a href="/login" className="text-foreground border-b border-foreground pb-0.5 hover:text-primary hover:border-primary transition-colors">로그인하기</a>
                    </div>
                </div>
            </motion.div>
        </div>
